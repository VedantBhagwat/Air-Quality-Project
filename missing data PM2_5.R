...
md_plot_PM2_5_2018<-ggplot(data=missing_data_PM2_5_2018, aes(x=Station, y=value)) +
...
md_plot_PM2_5_2019<-ggplot(data=missing_data_PM2_5_2019, aes(x=Station, y=value)) +
  geom_bar(stat="identity", fill="steelblue")
md_plot_PM2_5_2019 + 
  # geom_text(aes(label=value), vjust=1.6, color="white", size=3.5)+
  geom_text(aes(label=value), vjust=-0.3, size=3.5)+
...
md_plot_PM2_5_2020<-ggplot(data=missing_data_PM2_5_2020, aes(x=Station, y=value)) +
  geom_bar(stat="identity", fill="steelblue")
md_plot_PM2_5_2020 + 
  # geom_text(aes(label=value), vjust=1.6, color="white", size=3.5)+
  geom_text(aes(label=value), vjust=-0.3, size=3.5)+
...
X2020_Hourly_PM2_5_for_UCC <- X2020_Hourly_PM2_5_for_UCC[,c(T,(nrow(X2020_Hourly_PM2_5_for_UCC)*0.25 > missing_data_PM2_5_2020$value))]
# reduced from 29 columns to  27 columns
...
library(tidyr)
X2018_Hourly_PM2_5_for_UCC[,-1] %>% gather() %>% head()
ggplot(gather(X2018_Hourly_PM2_5_for_UCC[,-1]), aes(value)) + 
  geom_histogram(bins = 10) + 
  facet_wrap(~key, scales = 'free_x')

X2019_Hourly_PM2_5_for_UCC[,-1] %>% gather() %>% head()
ggplot(gather(X2019_Hourly_PM2_5_for_UCC[,-1]), aes(value)) + 
  geom_histogram(bins = 10) + 
  facet_wrap(~key, scales = 'free_x')

X2020_Hourly_PM2_5_for_UCC[,-1] %>% gather() %>% head()
ggplot(gather(X2020_Hourly_PM2_5_for_UCC[,-1]), aes(value)) + 
  geom_histogram(bins = 10) + 
  facet_wrap(~key, scales = 'free_x')


########### Boxplots ###########
boxplot(X2018_Hourly_PM2_5_for_UCC[,-1])
# Some negative values present in 2018
boxplot(X2019_Hourly_PM2_5_for_UCC[,-1])
boxplot(X2020_Hourly_PM2_5_for_UCC[,-1])



# Check number of NA's
sum(is.na.data.frame(X2018_Hourly_PM2_5_for_UCC)) # 1848
sum(is.na.data.frame(X2019_Hourly_PM2_5_for_UCC)) # 2522
sum(is.na.data.frame(X2020_Hourly_PM2_5_for_UCC)) # 2076
...
# which(is.na(X2020_Hourly_PM2_5_for_UCC), arr.ind=TRUE)
# OR
# library(tidyverse)
# X2020_Hourly_PM2_5_for_UCC %>%
#   rowid_to_column() %>%
#   filter(is.na(X2020_Hourly_PM2_5_for_UCC))
